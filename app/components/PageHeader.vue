  ...
  script: [
    {
      ...
      innerHTML: JSON.stringify({
        ...
        itemListElement: props.breadcrumbs.map((crumb, idx) => ({
          ...
          item:
            crumb.path === "/"
              ? "https://drniharmodi.com/"
              : `https://drniharmodi.com${crumb.path}`,
        })),
      }),
    },
  ],
...
<template>
  ...
  >
    ...
    <div
      class="container mx-auto relative z-10 flex flex-col items-center justify-center text-center"
    >
      <h1
        class="text-4xl md:text-6xl font-mirage text-secondary leading-tight mb-0 page-header-title"
      >
        {{ title }}
      </h1>
      <p
        v-if="subtitle"
        class="text-base md:text-lg max-w-4xl mx-auto text-gray-600 font-medium tracking-wide page-header-subtitle mb-2"
      >
        {{ subtitle }}
      </p>
      ...
      <!-- Breadcrumbs -->
      <nav
        class="page-header-breadcrumb flex items-center gap-2 text-sm font-medium text-gray-600 mt-4"
