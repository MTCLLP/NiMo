    ...
    >
      ...
      >
        <div class="flex items-center gap-4">
          <div v-if="item.icon" class="w-12 h-12 flex-shrink-0 flex items-center justify-center rounded-full bg-accent/20 text-primary" v-html="item.icon"></div>
          <slot name="item-title" :item="item" :index="idx">
            <span class="text-lg font-medium text-gray-800" :class="{'text-2xl md:text-3xl font-mirage': item.icon}">{{ item.title }}</span>
          </slot>
        </div>
        <svg 
          class="w-5 h-5 text-gray-400 transform transition-transform duration-300 flex-shrink-0 ml-4" 
      ...
      <div v-show="item.open" class="px-6 pb-6 text-gray-500 text-sm leading-relaxed">
