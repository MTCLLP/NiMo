<script setup lang="ts">
import { useRoute } from 'vue-router';
import { proceduresData } from '~/data/procedures-content';
import { computed } from 'vue';

const route = useRoute();
const procedureSlug = route.params.slug as string;

const procedure = computed(() => {
  return proceduresData.find(p => p.slug === procedureSlug);
});

// Setting meta tags dynamically based on the procedure
useHead({
  title: procedure.value ? `${procedure.value.name} in Mumbai - Dr. Nihar Modi` : 'Procedure Not Found',
  meta: [
    { 
      name: 'description', 
      content: procedure.value ? procedure.value.intro.substring(0, 150) + '...' : 'Explore orthopaedic procedures performed by Dr. Nihar Modi.'
    }
  ]
});
</script>

<template>
  <main>
    <template v-if="procedure">
      <PageHeader 
        :title="procedure.name" 
        :breadcrumbs="[
          { name: 'Home', path: '/' }, 
          { name: 'Procedures', path: '/procedures' },
          { name: procedure.name, path: `/procedures/${procedure.slug}` }
        ]"
      />

      <section class="py-16 md:py-24 bg-white">
        <div class="container mx-auto px-6 max-w-4xl">
          
          <div class="space-y-16">
            <!-- Introduction -->
            <div>
              <p class="text-lg md:text-xl text-gray-700 leading-relaxed font-light">
                {{ procedure.intro }}
              </p>
            </div>

            <!-- Symptoms -->
            <div>
              <h2 class="text-3xl font-mirage text-primary pb-4">Indications for Surgery</h2>
              <ul class="space-y-4">
                <li v-for="(sym, idx) in procedure.symptoms" :key="idx" class="flex items-start gap-3 text-gray-700">
                  <svg class="w-5 h-5 text-primary flex-shrink-0 mt-0.5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M5 13l4 4L19 7"></path></svg>
                  <span class="leading-relaxed">{{ sym }}</span>
                </li>
              </ul>
            </div>

            <!-- Diagnosis -->
            <div>
              <h2 class="text-3xl font-mirage text-primary pb-4">Pre-operative Evaluation</h2>
              <p class="text-gray-700 leading-relaxed">{{ procedure.diagnosis }}</p>
            </div>

            <!-- Treatment Options -->
            <div>
              <h2 class="text-3xl font-mirage text-primary pb-4">Surgical Options</h2>
              <ul class="space-y-4">
                <li v-for="(trt, idx) in procedure.treatments" :key="idx" class="flex items-start gap-3 text-gray-700">
                  <svg class="w-5 h-5 text-primary flex-shrink-0 mt-0.5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M5 13l4 4L19 7"></path></svg>
                  <span class="leading-relaxed">{{ trt }}</span>
                </li>
              </ul>
            </div>
            
            <!-- Management and Recovery -->
            <div>
              <h2 class="text-3xl font-mirage text-primary pb-4">Recovery & Rehabilitation</h2>
              <p class="text-gray-700 leading-relaxed">{{ procedure.management }}</p>
            </div>

            <!-- FAQs -->
            <div>
              <h2 class="text-3xl font-mirage text-primary pb-4">Frequently Asked Questions</h2>
              <div class="space-y-6">
                <div v-for="(faq, idx) in procedure.faqs" :key="idx" class="bg-white border border-gray-200 rounded-xl p-6 shadow-sm">
                  <h4 class="text-lg font-bold text-gray-900 mb-3">{{ faq.question }}</h4>
                  <p class="text-gray-600 leading-relaxed">{{ faq.answer }}</p>
                </div>
              </div>
            </div>
          </div>

        </div>
      </section>
      
      <CtaSection force-show>
        <template #title>
          Considering <br class="hidden md:block" /><span class="text-primary italic">{{ procedure.name.toLowerCase() }}?</span>
        </template>
        <template #desc>
          Book a consultation today for an expert evaluation and to discuss if surgery is the right option for you.
        </template>
      </CtaSection>
    </template>
    
    <template v-else>
      <div class="min-h-[60vh] flex flex-col items-center justify-center text-center px-4">
        <h1 class="text-4xl font-mirage text-primary mb-4">Procedure Not Found</h1>
        <p class="text-gray-600 mb-8">We could not find the information you are looking for.</p>
        <NuxtLink to="/" class="bg-primary text-white px-8 py-4 rounded-full font-medium hover:bg-primary/90 transition-colors">
          Return Home
        </NuxtLink>
      </div>
    </template>
  </main>
</template>
